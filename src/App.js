...
export default function App() {
  ...
  return (
    <div className="App">
      ...
      <Collapse collapsed={true} label="gridCorner">
        ...
        <Collapse collapsed={true} label="renderGridCorner">
          <GridTable
            ...
            config={{
              gridCorner: {
                renderGridCorner: ({ rows, cols }) => {
                  return (
                    <div>
                      renderTL {rows?.length}, {cols?.length}
                    </div>
                  );
                }
              }
      ...
      <Collapse collapsed={true} label="gridCell2">
        <Collapse collapsed={true} label="renderGridCell2">
          <GridTable
            rows={[...rows, 6, 7, 8]}
            cols={[...cols, 8, 9]}
            items={items}
        ...
        <Collapse collapsed={true} label="GridCell2Component">
          <GridTable
            rows={[...rows, 6, 7, 8]}
            cols={[...cols, 8, 9]}
            items={items}
            config={{
              gridCell2: {
                GridCell2Component: Cell2Component
              }
            }}
          />
        </Collapse>
